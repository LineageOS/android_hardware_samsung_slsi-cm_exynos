...
void ExynosOverlayDisplay::configureOtfWindow(hwc_rect_t &displayFrame,
        int32_t blending, int32_t planeAlpha, int format, 
#ifdef DECON_FB
        decon_win_config &cfg)
#else
...
void ExynosOverlayDisplay::configureHandle(private_handle_t *handle,
        ...
        int32_t blending, int32_t planeAlpha, int fence_fd, 
#ifdef DECON_FB
        decon_win_config &cfg)
#else
        s3c_fb_win_config &cfg)
...
void ExynosOverlayDisplay::configureOverlay(hwc_layer_1_t *layer, 
#ifdef DECON_FB
        decon_win_config &cfg)
#else
        s3c_fb_win_config &cfg)
...
int ExynosOverlayDisplay::postGscM2M(hwc_layer_1_t &layer, 
#ifdef DECON_FB
        struct decon_win_config *config,
#else
        struct s3c_fb_win_config *config,
#endif
        int win_map, int index)
{
    exynos5_hwc_post_data_t *pdata = &mPostData;
...
int ExynosOverlayDisplay::postGscOtf(hwc_layer_1_t &layer, 
#ifdef DECON_FB
        struct decon_win_config *config,
#else
        struct s3c_fb_win_config *config,
...
void ExynosOverlayDisplay::handleStaticLayers(hwc_display_contents_1_t *contents, 
#ifdef DECON_FB
        struct decon_win_config_data &win_data,
#else
        struct s3c_fb_win_config_data &win_data,
        ...
        int tot_ovly_wins)
{
    ...
    memcpy(&win_data.config[win_map], &mLastConfig[win_map], 
#ifdef DECON_FB
            sizeof(struct decon_win_config));
#else
            sizeof(struct s3c_fb_win_config));
#endif

...
class ExynosOverlayDisplay : public ExynosDisplay {
    public:
        ...
        struct fb_win_config mLastConfig[NUM_HW_WINDOWS];
    ...
    protected:
        ...
        int postGscOtf(hwc_layer_1_t &layer, struct fb_win_config *config,
                int win_map, int index);
        void handleStaticLayers(hwc_display_contents_1_t *contents, struct fb_win_config_data &win_data,
                int tot_ovly_wins);
        void cleanupGscs();

        virtual int postGscM2M(hwc_layer_1_t &layer, struct fb_win_config *config,
                int win_map, int index);

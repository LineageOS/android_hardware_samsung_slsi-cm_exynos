...
class ExynosOverlayDisplay : public ExynosDisplay {
    ...
    protected:
        ...
        int postGscOtf(hwc_layer_1_t &layer, fb_win_config *config,
                int win_map, int index);
        void handleStaticLayers(hwc_display_contents_1_t *contents, fb_win_config_data &win_data,
                int tot_ovly_wins);
        void cleanupGscs();

        virtual int postGscM2M(hwc_layer_1_t &layer, fb_win_config *config,
                int win_map, int index);
